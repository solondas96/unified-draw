...
import { Copy, ClipboardPaste, CopyPlus, Trash2, ArrowUp, ArrowDown, Lock, Unlock, Edit3 } from "lucide-react";
...
export const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, onClose }) => {
  ...
  const hasText = elements.some(e => selectedIds.includes(e.id) && (e.type === "text" || e.shapeType === "rectangle" || e.shapeType === "circle" || e.type === "shape"));
  ...
  const MenuItem = ({ icon: Icon, label, shortcut, onClick, danger = false }: any) => (
    ...
    >
      ...
      {shortcut && <span className="text-[10px] text-slate-500">{shortcut}</span>}
    </button>
  );
  ...
  return (
    ...
    >
      ...
      <MenuItem icon={ClipboardPaste} label="Paste" shortcut="Ctrl+V" onClick={paste} />
      <MenuItem icon={CopyPlus} label="Duplicate" shortcut="Ctrl+D" onClick={duplicateSelected} />
      <MenuItem icon={Trash2} label="Delete" shortcut="Del" danger onClick={deleteSelected} />
      <Divider />
      <MenuItem icon={ArrowUp} label="Bring to Front" shortcut="Ctrl+Shift+↑" onClick={() => selectedIds.forEach(bringToFront)} />
      <MenuItem icon={ArrowDown} label="Send to Back" shortcut="Ctrl+Shift+↓" onClick={() => selectedIds.forEach(sendToBack)} />
      <Divider />
      <MenuItem icon={anyUnlocked ? Lock : Unlock} label={anyUnlocked ? "Lock" : "Unlock"} shortcut="Ctrl+L" onClick={toggleLockSelected} />
