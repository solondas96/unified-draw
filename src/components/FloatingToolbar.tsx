...
import { Copy, ClipboardPaste, Bold, Italic, Underline, Type, AlignLeft, AlignCenter, AlignRight, AlignJustify } from "lucide-react";
import { COLOR_PALETTE } from "./Sidebar";
import type { Element } from "../types";

export const FloatingToolbar: React.FC = () => {
  const { elements, selectedIds, zoom, panX, panY, updateElement, copy, paste } = useStore();
  ...
  const safeMenuX = Math.max(20, Math.min(position.x - 200, window.innerWidth - 450)); // Approximate centering
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-1 border-r border-slate-700 pr-2">
        <button onClick={copy} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded" title="Copy">
          <Copy className="w-4 h-4" />
        </button>
        <button onClick={paste} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded" title="Paste">
        ...
        <>
          ...
          <div className="flex items-center space-x-1 border-r border-slate-700 pr-2 pl-1">
            <select
              ...
              onChange={(e) => handleUpdate({ textStyle: { ...ts!, fontFamily: e.target.value } })}
            ...
            <input
              ...
              onChange={(e) => handleUpdate({ textStyle: { ...ts!, fontSize: Number(e.target.value) } })}
          ...
          <div className="flex items-center space-x-1 border-r border-slate-700 pr-2 pl-1">
            <button
              onClick={() => handleUpdate({ textStyle: { ...ts!, fontWeight: ts?.fontWeight === 'bold' ? 'normal' : 'bold' } })}
            ...
            <button
              onClick={() => handleUpdate({ textStyle: { ...ts!, fontStyle: ts?.fontStyle === 'italic' ? 'normal' : 'italic' } })}
            ...
            <button
              onClick={() => handleUpdate({ textStyle: { ...ts!, textDecoration: ts?.textDecoration === 'underline' ? 'none' : 'underline' } })}
          ...
          <div className="flex items-center space-x-1 pl-1 relative group">
            <button className="flex items-center space-x-1 p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded">
              ...
              <div className="w-3 h-3 rounded-full border border-slate-600" style={{ backgroundColor: ts?.color || '#ffffff' }} />
            </button>
