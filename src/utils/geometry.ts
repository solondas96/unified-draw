...
export function getCurvedPath(start: Point, end: Point): string {
  ...
  const offset1 = Math.abs(dx) > Math.abs(dy) ? Math.abs(dx) * tension : Math.abs(dy) * tension;
  const offset2 = offset1;
  
  // Modify cp1 based on side
  if (start.side === 0) cp1.y -= offset1; // Top
  else if (start.side === 2) cp1.x += offset1; // Right
  else if (start.side === 4) cp1.y += offset1; // Bottom
  else if (start.side === 6) cp1.x -= offset1; // Left
  // Corners just project outwards diagonally
  else if (start.side === 1) { cp1.x += offset1*0.7; cp1.y -= offset1*0.7; }
  else if (start.side === 3) { cp1.x += offset1*0.7; cp1.y += offset1*0.7; }
  else if (start.side === 5) { cp1.x -= offset1*0.7; cp1.y += offset1*0.7; }
  else if (start.side === 7) { cp1.x -= offset1*0.7; cp1.y -= offset1*0.7; }
  ...
  else if (end.side === 1) { cp2.x += offset2*0.7; cp2.y -= offset2*0.7; }
  else if (end.side === 3) { cp2.x += offset2*0.7; cp2.y += offset2*0.7; }
  else if (end.side === 5) { cp2.x -= offset2*0.7; cp2.y += offset2*0.7; }
  else if (end.side === 7) { cp2.x -= offset2*0.7; cp2.y -= offset2*0.7; }
