...
async function getDB(): Promise<IDBPDatabase | null> {
  if (!dbPromise) {
    dbPromise = openDB(DB_NAME, DB_VERSION, {
      ...
      Logger.warn(
        "IndexedDB is not available. Changes will not be saved locally.",
        error,
      );
      return null;
    });
...
export async function saveCanvas(canvas: Canvas): Promise<void> {
  try {
    ...
    Logger.error("Failed to save canvas to IndexedDB:", error);
  }
}
...
export async function loadCanvas(id: string): Promise<Canvas | undefined> {
  try {
    ...
    Logger.error("Failed to load canvas from IndexedDB:", error);
    return undefined;
  }
}
...
export async function listCanvases(): Promise<Canvas[]> {
  try {
    ...
    Logger.error("Failed to list canvases from IndexedDB:", error);
    return [];
  }
}
...
export async function deleteCanvas(id: string): Promise<void> {
  try {
    ...
    Logger.error("Failed to delete canvas from IndexedDB:", error);
  }
}
...
export async function getLatestCanvas(): Promise<Canvas | undefined> {
  try {
    ...
    Logger.error("Failed to get latest canvas:", error);
    return undefined;
  }
}
